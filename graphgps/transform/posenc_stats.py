...
from graphgps.transform.cy2c_utils import make_NEWDATA
...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    'CY2C': Converting clique to Cycle and removing edge not included in any clique.
    ...
    for t in pe_types:
        if t not in ['LapPE', 'EquivStableLapPE', 'SignNet', 'RWSE', 'HKdiagSE',
                     'HKfullPE', 'ElstaticSE', 'GraphormerBias','CY2C']:
    ...
    if 'CY2C' in pe_types:
        cycle_index, cycle_attr, pos_edge_attr = make_NEWDATA(
            data.clone(),
            cfg.max_num_nodes,
            cy2c_self=cfg.cy2.cy2c_self,
            cy2c_same_attr=cfg.cy2.cy2c_same_attr,
            cy2c_trans=cfg.cy2.cy2c_trans
        )
        data.cycle_index=cycle_index
        data.cycle_attr=cycle_attr
