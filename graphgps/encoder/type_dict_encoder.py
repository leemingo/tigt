...
@register_edge_encoder('TypeDictCycle')
class TypeDictCycleEncoder(torch.nn.Module):
    def __init__(self, emb_dim):
        super().__init__()

        if cfg.dataset.edge_encoder_num_types < 1:
            num_types = 2
        else : 
            num_types = cfg.dataset.edge_encoder_num_types
    ...
    def forward(self, batch):       
        batch.pos_edge_attr = self.encoder(batch.pos_edge_attr)
        batch.cycle_attr = self.encoder(batch.cycle_attr)
        return batch
